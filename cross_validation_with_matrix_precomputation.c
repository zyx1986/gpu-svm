...
void run_pair(struct svm_problem * p_km, svm_problem* prob, svm_parameter* param, int nr_fold  )
{
...
void do_cross_validation_with_KM_precalculated( svm_problem* prob, svm_parameter* param, int nr_fold )
{
    struct svm_problem p_km;

    setup_pkm(&p_km, prob);

    run_pair(&p_km, prob, param, nr_fold);
